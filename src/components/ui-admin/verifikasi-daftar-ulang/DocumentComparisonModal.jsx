    ...
    const SingleViewer = ({ doc }) => {
        ...
        return (
            <div className="w-full h-full bg-gray-100 rounded-lg overflow-hidden border border-gray-200 relative group">
                {isImage && (
                    <div className="absolute top-4 left-1/2 -translate-x-1/2 rounded-lg p-1.5 flex items-center gap-1 z-20 transition-opacity opacity-0 group-hover:opacity-100">
                ...
                <div className="w-full h-full overflow-auto flex items-center justify-center p-4">
                    {isImage ? (
                        ...
                        <div style={{ transform: `scale(${zoom / 100}) rotate(${rotation}deg)`, transition: 'transform 0.2s ease-out' }} className="pointer-events-auto">
                            <img src={doc.path_file} alt={doc.nama_file} className="max-w-none shadow-lg origin-center" style={{ maxHeight: zoom === 100 ? '100%' : 'none', maxWidth: zoom === 100 ? '100%' : 'none' }} /> {/* modify zoom */}
                        </div>
                    ) : isPdf ? (
